...
def download() {
       sh encoding: 'UTF-8', 
       script: '''
       API_KEY=`cat /run/secrets/secret_api_key`;
       /usr/sbin/DownloadLatestBuild --api_key ${API_KEY} --dest_path /var/builds --product StarOS --platform qvpc-di --release ${RELEASE} 
       '''
}